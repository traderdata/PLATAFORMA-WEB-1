    ...
    internal class Index
    {
      ...
      public Recordset MoneyFlowIndex(Navigator pNav, Recordset pOHLCV, int periods, string Alias)
      {
        ...
        for (Record = Start; Record < RecordCount + 2; Record++)
        {
          ...
          {
            pNav.MovePrevious();
            double Price1 = (pOHLCV.GetField("Maximo").Value(pNav.Position).Value +
                             pOHLCV.GetField("Minimo").Value(pNav.Position).Value +
                             pOHLCV.GetField("Ultimo").Value(pNav.Position).Value) / 3;
            pNav.MoveNext();
            double V = pOHLCV.GetField("Volume").Value(pNav.Position).Value;
            if (V < 1) { V = 1; }
            double Price2 = (pOHLCV.GetField("Maximo").Value(pNav.Position).Value +
                             pOHLCV.GetField("Minimo").Value(pNav.Position).Value +
                             pOHLCV.GetField("Ultimo").Value(pNav.Position).Value) / 3;
      ...
      public Recordset SwingIndex(Navigator pNav, Recordset pOHLCV, double LimitMoveValue, string Alias)
      {
        ...
        for (Record = Start; Record < RecordCount + 1; Record++)
        {
          double Oy = pOHLCV.GetField("Abertura").Value(pNav.Position - 1).Value;
          double Ot = pOHLCV.GetField("Abertura").Value(pNav.Position).Value;
          double Ht = pOHLCV.GetField("Maximo").Value(pNav.Position).Value;
          double Lt = pOHLCV.GetField("Minimo").Value(pNav.Position).Value;
          double Cy = pOHLCV.GetField("Ultimo").Value(pNav.Position - 1).Value;
          double Ct = pOHLCV.GetField("Ultimo").Value(pNav.Position).Value;
      ...
      public Recordset ChaikinMoneyFlow(Navigator pNav, Recordset pOHLCV, int periods, string Alias)
      {
        ...
        for (Record = periods + 1; Record < RecordCount + 1; Record++)
        {
          ...
          {
            double? a = ((pOHLCV.Value("Ultimo", Record - n) - pOHLCV.Value("Minimo", Record - n)) -
                        (pOHLCV.Value("Maximo", Record - n) - pOHLCV.Value("Ultimo", Record - n)));
            double? b = (pOHLCV.Value("Maximo", Record - n) - pOHLCV.Value("Minimo", Record - n)) *
      ...
      public Recordset StochasticMomentumIndex(Navigator pNav, Recordset pOHLCV, int KPeriods, int KSmooth, int KDoubleSmooth,
        int DPeriods, EnumGeral.IndicatorType MAType, EnumGeral.IndicatorType PctD_MAType)
      {
        MovingAverage MA = new MovingAverage();
        Recordset Results = new Recordset();
        General GN = new General();
        ...
        Recordset Temp = GN.HHV(pNav, pOHLCV.GetField("Maximo"), KPeriods, "HHV");
        Field HHV = new Field(RecordCount, "HHV");
        Temp.CopyField(HHV, "HHV");

        Temp = GN.LLV(pNav, pOHLCV.GetField("Minimo"), KPeriods, "LLV");
        ...
        for (Record = 1; Record != RecordCount + 1; ++Record)
        {
          Value = pOHLCV.Value("Ultimo", Record) - (0.5f * (HHV.Value(Record) + LLV.Value(Record)));
        ...
        int StandardDeviations, string Alias)
      {
        ...
        Recordset Field2 = Stdv.StandardDeviation(pNav, Field1, periods, StandardDeviations, EnumGeral.IndicatorType.MediaMovelSimples, "STDV");
