    ...
    internal class Oscillator
    {
            ...
            int Cycle1, int Cycle2, int Cycle3, string Alias)
      {
        ...
        for (iRecord = iStart; iRecord < iRecordCount + 2; iRecord++)
        {
          ...
          {
            TL = pOHLCV.GetField("Minimo").Value(pNav.Position) <
                 pOHLCV.GetField("Ultimo").Value(pNav.Position - 1)
                   ? pOHLCV.GetField("Minimo").Value(pNav.Position)
                   : pOHLCV.GetField("Ultimo").Value(pNav.Position - 1);
            BP = pOHLCV.GetField("Ultimo").Value(pNav.Position) - TL;
            TR = pOHLCV.GetField("Maximo").Value(pNav.Position) -
                    pOHLCV.GetField("Minimo").Value(pNav.Position);
            if (TR < pOHLCV.GetField("Maximo").Value(pNav.Position) -
                    pOHLCV.GetField("Ultimo").Value(pNav.Position - 1))
            {
              TR = pOHLCV.GetField("Maximo").Value(pNav.Position) -
                      pOHLCV.GetField("Ultimo").Value(pNav.Position - 1);
            }
            if (TR < pOHLCV.GetField("Ultimo").Value(pNav.Position - 1) -
                    pOHLCV.GetField("Minimo").Value(pNav.Position))
            {
              TR = pOHLCV.GetField("Ultimo").Value(pNav.Position - 1) -
                      pOHLCV.GetField("Minimo").Value(pNav.Position);
            }
            BPSum1 += BP;
            TRSum1 += TR;
      ...
      public Recordset WilliamsPctR(Navigator pNav, Recordset pOHLCV, int periods, string Alias)
      {
        ...
        for (Record = Start; Record < RecordCount + 2; Record++)
        {
          ...
          double? HH = 0;
          double? LL = pOHLCV.GetField("Minimo").Value(pNav.Position);
          ...
          {
            if (pOHLCV.GetField("Maximo").Value(pNav.Position) > HH)
            {
              HH = pOHLCV.GetField("Maximo").Value(pNav.Position);
            }
            if (pOHLCV.GetField("Minimo").Value(pNav.Position) < LL)
            {
              LL = pOHLCV.GetField("Minimo").Value(pNav.Position);
            }
            pNav.MoveNext();
          }//Period

          pNav.MovePrevious();
          Field1.Value(pNav.Position, ((HH - pOHLCV.GetField("Ultimo").Value(pNav.Position)) / (HH - LL)) * -100);
          pNav.MoveNext();
      ...
      public Recordset WilliamsAccumulationDistribution(Navigator pNav, Recordset pOHLCV, string Alias)
      {
        ...
        for (Record = 1; Record < RecordCount; Record++)
        {
          ...
          double TRH = pOHLCV.GetField("Ultimo").ValueEx(pNav.Position - 1);
          if (pOHLCV.GetField("Maximo").ValueEx(pNav.Position) > TRH)
          {
            TRH = pOHLCV.GetField("Maximo").ValueEx(pNav.Position);
          }

          double TRL = pOHLCV.GetField("Ultimo").ValueEx(pNav.Position - 1);
          if (pOHLCV.GetField("Minimo").ValueEx(pNav.Position) < TRL)
          {
            TRL = pOHLCV.GetField("Minimo").ValueEx(pNav.Position);
          }

          double Value;
          if (pOHLCV.GetField("Ultimo").ValueEx(pNav.Position) > pOHLCV.GetField("Ultimo").ValueEx(pNav.Position - 1))
          {
            Value = pOHLCV.GetField("Ultimo").ValueEx(pNav.Position) - TRL;
          }
          else if (pOHLCV.GetField("Ultimo").ValueEx(pNav.Position) < pOHLCV.GetField("Ultimo").ValueEx(pNav.Position - 1))
          {
            Value = pOHLCV.GetField("Ultimo").ValueEx(pNav.Position) - TRH;
          }
          else
          {
            Value = 0;
      ...
      ///<returns>Recordset</returns>
      public Recordset ChaikinVolatility(Navigator pNav, Recordset pOHLCV, int periods, int ROC, EnumGeral.IndicatorType MAType)
      ...
      public Recordset ChaikinVolatility(Navigator pNav, Recordset pOHLCV, int periods, int ROC, EnumGeral.IndicatorType MAType, string Alias)
      {
        MovingAverage MA = new MovingAverage();
        Recordset Results = new Recordset();
        Recordset HLMA = null;
        ...
        for (Record = 1; Record < RecordCount + 1; Record++)
        {
          HL.Value(pNav.Position, pOHLCV.GetField("Maximo").Value(pNav.Position) -
          pOHLCV.GetField("Minimo").Value(pNav.Position));
          pNav.MoveNext();
        }
      ...
      public Recordset StochasticOscillator(Navigator pNav, Recordset pOHLCV, int KPeriods, int KSlowingPeriods, int DPeriods,
        EnumGeral.IndicatorType MAType)
      {
        MovingAverage MA = new MovingAverage();
        Recordset PctD = null;
        Recordset Results = new Recordset();
        ...
        for (Record = Start; Record < RecordCount + 2; Record++)
        {
          ...
          double? HH = pOHLCV.GetField("Maximo").Value(pNav.Position);
          double? LL = pOHLCV.GetField("Minimo").Value(pNav.Position);
          ...
          pNav.MovePrevious();
          double? Value = (pOHLCV.GetField("Ultimo").Value(pNav.Position) - LL) / (HH - LL) * 100;
      ...
      ///<exception cref="ArgumentNullException"></exception>
      public Recordset PriceOscillator(Navigator pNav, Field pSource, int LongCycle, int ShortCycle,
        EnumGeral.IndicatorType MAType, string Alias)
      {
      ...
      public Recordset MACD(Navigator pNav, Recordset pOHLCV, int SignalPeriods, int LongCycle, int ShortCycle, string Alias)
      {
        ...
        Recordset EMA1 = MA.ExponentialMovingAverage(pNav, pOHLCV.GetField("Ultimo"), LongCycle, "EMA");
        Recordset EMA2 = MA.ExponentialMovingAverage(pNav, pOHLCV.GetField("Ultimo"), ShortCycle, "EMA");
        ...
        EnumGeral.IndicatorType MAType, string Alias)
      {
        ...
        for (Record = Start; Record < RecordCount + 1; Record++)
        {
          ...
          double? MPM = ((pOHLCV.Value("Maximo", pNav.Position) +
                         pOHLCV.Value("Minimo", pNav.Position)) / 2) -
                       ((pOHLCV.Value("Maximo", pNav.Position - 1) +
                         pOHLCV.Value("Minimo", pNav.Position - 1)) / 2);

          double? bd = (pOHLCV.Value("Maximo", pNav.Position) - pOHLCV.Value("Minimo", pNav.Position));
      ...
      public Recordset DirectionalMovementSystem(Navigator pNav, Recordset pOHLCV, int periods)
      {
        ...
        {
          double? high0 = pOHLCV.Value("Maximo", Record);
          double? high1 = pOHLCV.Value("Maximo", Record - 1);
          double? low1 = pOHLCV.Value("Minimo", Record - 1);
          double? low0 = pOHLCV.Value("Minimo", Record);
      ...
      public Recordset TrueRange(Navigator pNav, Recordset pOHLCV, string Alias)
      {
        ...
        for (Record = 2; Record != RecordCount + 1; ++Record)
        {
          ...
          double? T1 = pOHLCV.Value("Maximo", Record) - pOHLCV.Value("Minimo", Record);
          double T2 = Math.Abs((int)(pOHLCV.Value("Maximo", Record) - pOHLCV.Value("Ultimo", Record - 1)));
          double T3 = Math.Abs((int)(pOHLCV.Value("Ultimo", Record - 1) - pOHLCV.Value("Minimo", Record)));
      ...
      public Recordset Aroon(Navigator pNav, Recordset pOHLCV, int periods)
      {
        ...
        for (Record = periods + 1; Record != RecordCount + 1; ++Record)
        {
          ...
          double? HighestHigh = pOHLCV.Value("Maximo", Record);
          double? LowestLow = pOHLCV.Value("Minimo", Record);
          ...
          for (Period = Record - periods; Period != Record; ++Period)
          {
            ...
            if (pOHLCV.Value("Maximo", Period) > HighestHigh)
            {
              HighestHigh = pOHLCV.Value("Maximo", Period);
              HighPeriod = Period;
            }

            if (pOHLCV.Value("Minimo", Period) >= LowestLow) continue;
            LowestLow = pOHLCV.Value("Minimo", Period);
            LowPeriod = Period;
          } // Period
