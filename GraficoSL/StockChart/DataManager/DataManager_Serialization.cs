...
namespace Traderdata.Client.Componente.GraficoSL.StockChart.DataManager
{
  internal partial class DataManager
  {
    ...
    internal void SaveToXml(XElement xRoot)
    {
      ...
      {
          if (entry.Series.OHLCType == EnumGeral.TipoSerieOHLC.Desconhecido) continue;
        ...
        attribute = new XAttribute("Index", entry.Series._seriesIndex.ToString());
        xmlSeries.Add(attribute);
    ...
    internal void LoadFromXml(XElement xRoot)
    {
      ...
      _chart.indexFinal = _timestamps.Count;
      ...
      foreach (XElement childNode in xmlOhlcData.Elements())
      {
        ...
        string symbol = childNode.Attributes("Name").First().Value;
        EnumGeral.TipoSerieOHLC ohlcType = (EnumGeral.TipoSerieOHLC)System.Enum.Parse(typeof(EnumGeral.TipoSerieOHLC),
