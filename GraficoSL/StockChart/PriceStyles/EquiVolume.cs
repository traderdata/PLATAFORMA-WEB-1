...
namespace Traderdata.Client.Componente.GraficoSL.StockChart.PriceStyles
{
  internal partial class EquiVolume : Style
  {
    ...
    public EquiVolume(Traderdata.Client.Componente.GraficoSL.StockChart.Stock stock)
      : base(stock)
    {
    }
    ...
    public override bool Paint()
    {
      ...
      */
      ...
      Series volume = chartX.GetSeriesOHLCV(_series, EnumGeral.TipoSerieOHLC.Volume);
      ...
      Brush upBrush = new SolidColorBrush(_series._upColor.HasValue ? _series._upColor.Value : chartX.CandleCorAlta);
      Brush downBrush = new SolidColorBrush(_series._downColor.HasValue ? _series._downColor.Value : chartX.CandleCorBaixa);
      Brush upGradBrush = Utils.CreateFadeVertBrush(_series._upColor.HasValue ? _series._upColor.Value : chartX.CandleCorAlta,
                                                    Colors.Black);
      Brush downGradBrush = Utils.CreateFadeVertBrush(Colors.Black,
                                                      _series._downColor.HasValue
                                                        ? _series._downColor.Value
                                                        : chartX.CandleCorBaixa);
      ...
      for (n = chartX.indexInicial; n < chartX.indexFinal; n++)
      {
        if (!open[n].Value.HasValue || !high[n].Value.HasValue || !low[n].Value.HasValue || !close[n].Value.HasValue ||
            !volume[n].Value.HasValue) continue;
        ...
        if (!chartX.darwasBoxes) continue;
      ...
      for (n = chartX.indexInicial; n < chartX.indexFinal; n++)
      {
        ...
        if (chartX.estiloPreco == EnumGeral.EstiloPrecoEnum.EquiVolumeShadow)
        {
        ...
        if (chartX.estiloPreco == EnumGeral.EstiloPrecoEnum.CandleVolume)
        {
          if (close[n].Value > open[n].Value)
          {
            box.Top = _series._chartPanel.GetY(close[n].Value.Value);
