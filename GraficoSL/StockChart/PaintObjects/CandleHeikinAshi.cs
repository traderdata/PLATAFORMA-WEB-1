...
namespace Traderdata.Client.Componente.GraficoSL.StockChart.PaintObjects
{
  internal partial class CandleHeikinAshi : IPaintObject
  {
    ...
    private void CalculateXValues()
    {
      ...
      _xOpen = (double)(_index + chartX.indexInicial == 0 ? _open.Value :
        (_high._collectionOwner[_index + chartX.indexInicial - 1].Value + _low._collectionOwner[_index + chartX.indexInicial - 1].Value) / 2);
