...
//    private static Rectangle _hitRect;
    public static FrameworkElement HitTest(this Canvas self, Point p, Type []neededObjects)
    {
#if WPF
      for (int dx = -HitTestGap; dx <= HitTestGap; dx++)
        for (int dy = -HitTestGap; dy <= HitTestGap; dy++)
        {
          ...
          foreach (Type type in neededObjects)
          {
            if (frameworkElementType.IsSubclassOf(type) || frameworkElementType.Equals(type))
              return (FrameworkElement)hitTestResult.VisualHit;
          }
        }
#endif
#if SILVERLIGHT
      ...
      foreach (FrameworkElement element in VisualTreeHelper.FindElementsInHostCoordinates(new Rect(point.X, point.Y, hitRect.Width, hitRect.Height), self))
      {
        if (element.Tag == null) continue;
        Type frameworkElementType = element.Tag.GetType();
        foreach (Type type in neededObjects)
        {
          if (frameworkElementType.IsSubclassOf(type) || frameworkElementType.Equals(type))
            return element;
    ...
    public static IEnumerable<FrameworkElement> FintElementsFromRect(Canvas root, Rect rect)
    {
      ...
      {
        foreach (Point corner in child.GetCanvasCorners())
        {
  ...
  internal static class StringBuilderExtensions
  {
    ...
    public static Geometry GetDataPathGeometry(this StringBuilder self)
    {
#if WPF
      if (self.Length == 0)
        return Geometry.Empty;
      return Geometry.Parse(self.ToString());
#endif
#if SILVERLIGHT
      if (self.Length == 0)
        return null;
