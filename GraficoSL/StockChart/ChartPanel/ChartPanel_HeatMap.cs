  ...
  internal class ChartPanel_HeatMap : ChartPanel
  {
    ...
    internal ChartPanel_HeatMap(StockChartX chartX)
    {
      ...
      ResourceDictionary resourceDictionary = new ResourceDictionary
                                                {
                                                  Source =
                                                    new Uri("/Traderdata.Client.Componente.GraficoSL.StockChart.StockChartX;component/Themes/Brushes.xaml",
    ...
    internal override void Paint()
    {
      ...
      double rectWidth = _chartX.PaintableWidth / (_chartX.indexFinal - _chartX.indexInicial);
      ...
      foreach (Series indicator in indicators)
      {
        ...
        indicator.DM.VisibleMinMax(indicator._seriesIndex, out min, out max);
        if (indicator.RecordCount == 0) continue;

        int cnt = 0;
        for (int i = _chartX.indexInicial; i < _chartX.indexFinal; i++, cnt++)
        {
          ...
          double x = cnt * rectWidth + _chartX.EspacoEsquerdaGrafico;
        ...
        Label lbl = Utils.DrawText(_chartX.EspacoEsquerdaGrafico + 10, indicatorIndex * panelHeight + 2,
