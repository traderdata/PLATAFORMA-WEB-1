    ...
    internal class GZipMessageEncoderFactory : MessageEncoderFactory
    {
        ...
        class GZipMessageEncoder : MessageEncoder
        {
            ...
            static ArraySegment<byte> CompressBuffer(ArraySegment<byte> buffer, BufferManager bufferManager, int messageOffset)
            {
                ...
                using (GZipOutputStream zipOut = new GZipOutputStream(memoryStream))
                {    
                    zipOut.Write(buffer.Array, buffer.Offset, buffer.Count);
                }
            ...
            static ArraySegment<byte> DecompressBuffer(ArraySegment<byte> buffer, BufferManager bufferManager)
            {
                ...
                using (GZipInputStream zipIn = new GZipInputStream(memoryStream))
                {
                    while (true)
                    {
                        int bytesRead = zipIn.Read(tempBuffer, 0, blockSize);
                        if (bytesRead == 0)
                            break;
            ...
            public override Message ReadMessage(System.IO.Stream stream, int maxSizeOfHeaders, string contentType)
            {
                ...
                return innerEncoder.ReadMessage(zipIn, maxSizeOfHeaders);
            }

            public override void WriteMessage(Message message, System.IO.Stream stream)
            {
                using (GZipOutputStream zipOut = new GZipOutputStream(stream))
                {
                    innerEncoder.WriteMessage(message, zipOut);
                }
